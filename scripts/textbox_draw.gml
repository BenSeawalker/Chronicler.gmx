...

{
    ...
    color_selected=c_black
    color_selection=make_color_rgb(51,153,255)
    color_marker=c_white
    if (i.color_text>-1) {color_normal=i.color_text; color_marker=i.color_text;}
    ...
    mouseover=(mx>=xx && mx<xx+w && my>=yy && my<yy+h)
    ...
    if (textbox_focus=i) {
        ...
        keyboard_string=""
        if(!i.read_only && key_press[vk_tab]) inserttext += "    ";
        ...
        // Delete
        if (deletetext!=0 || inserttext!="" || i.update) {
            i.update = false;
    ...
    for (l=i.start*!i.single_line l<i.lines l++) {
        ...
        if ((mouseover && textbox_select=-1) || textbox_select=i) {
            ...
            if ((mx>=xx || textbox_select=i) &&
                (mx<xx+w || textbox_select=i) &&
                (my>=yy+ly || (textbox_select=i && ly=0)) &&
                (my<yy+ly+hh || (textbox_select=i && (ly+lineheight>h || l=i.lines-1)))) {  // Cursor is inside line
                if (mouse_check_button(mb_left)) {
                    if (i.select_on_focus && textbox_lastfocus!=i) { // Select all
                        ...
                        textbox_select_clickline=0 textbox_select_clickpos=0
                        textbox_marker=current_time
                        if(!i.read_only)textbox_focus=i
                        keyboard_string=""
                        mouse_clear(mb_left)
                    } else {
                        textbox_select=i
    ...
    // Set cursor
    if (!i.read_only && (mouseover || textbox_select=i)) {
        textbox_mouseover=i
