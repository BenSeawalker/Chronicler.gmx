...
if(tbox.single_line)
{
    var txt = "";
    var ii = 1;
    if(string_width(tbox.text)>abs(x2-x1))
        while(string_width(txt+string_char_at(tbox.text,ii))<abs(x2-x1)) txt += string_char_at(tbox.text,ii++);
    else
        txt = tbox.text;
        
    draw_text_colour(x1,y1,txt,c,c,c,c,draw_get_alpha());
}
else
{
    var i=0;
